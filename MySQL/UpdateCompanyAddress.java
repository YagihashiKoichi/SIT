/**
 * データベース(animedb)にある指定された制作会社の本社住所を
 * １つ更新するクラス
...
public class UpdateCompanyAddress extends AbstractUpdater{
	
	/**
	 * (更新対象が複数ある場合)更新対象となりうるデータの一覧を番号をつけて
	 * 会社名、電話番号、更新前の本社住所を含めて表示
	 ...
	 * @param where 本社住所を更新したい制作会社の会社名を示すString
	 ...
	 * @param where 住所を更新したい制作会社の会社名を示すString
	 * @return 更新の条件に該当する制作会社の本社住所を更新する条件式
	 */
	public String updateMakeQuery(String set,String where,ResultSet result){
		String sql = "UPDATE 制作会社 SET 本社住所='" + set + "' WHERE 会社名='" + where + "'";
		return sql;
    }
	...
	public String reupdateMakeQuery(String set,ResultSet result){
		...
		try {
			...
			sql = "UPDATE 制作会社 SET 本社住所='" + set + "' WHERE 制作会社ID='" + id + "'";
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
