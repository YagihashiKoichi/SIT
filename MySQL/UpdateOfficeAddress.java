/**
 * データベース(animedb)にある指定された事務所の住所を
 * １つ更新するクラス
...
public class UpdateOfficeAddress extends AbstractUpdater{
	
	/**
	 * (更新対象が複数ある場合)更新対象となりうるデータの一覧を番号をつけて
	 * 事務所名、電話番号、更新前の住所を含めて表示
	 ...
	 * @param where 住所を更新したい事務所の事務所名を示すString
	 ...
	 * @param where 住所を更新したい事務所の事務所名を示すString
	 * @return 更新の条件に該当する事務所の住所を更新する条件式
	 */
	public String updateMakeQuery(String set,String where,ResultSet result){
		String sql = "UPDATE 事務所 SET 住所='" + set + "' WHERE 事務所名='" + where + "'";
		return sql;
    }
	...
	public String reupdateMakeQuery(String set,ResultSet result){
		...
		try {
			...
			sql = "UPDATE 事務所 SET 住所='" + set + "' WHERE 事務所ID='" + id + "'";
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
